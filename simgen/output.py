...
class statistics:
    ...
    def start(self,pop,year):
        ...
        for col in ['male','insch','educ','married','nkids','iso_smaf','risk_iso']:
            list_value_column.append(pop.hh[col].sort_values().unique())
        empty = pd.DataFrame(list(product(*list_value_column)),
                             columns=['age','male','insch','educ','married','nkids','iso_smaf','risk_iso']).set_index(['age','male','insch','educ','married','nkids','iso_smaf','risk_iso'])
        empty[year]=0.0
